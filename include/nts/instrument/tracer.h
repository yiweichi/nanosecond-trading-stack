...
struct alignas(64) TraceRecord {
    ...
    uint64_t latency_ticks(Hop from, Hop to) const {
        return ticks[static_cast<size_t>(to)]
             - ticks[static_cast<size_t>(from)];
    }

    uint64_t latency(Hop from, Hop to) const {
        return ticks_to_ns(latency_ticks(from, to));
    }
};
...
public:
    ...
    inline void start_trace() {
        if (__builtin_expect(!enabled_, 0)) return;
        in_trace_ = true;
        traces_[write_pos()].recorded_mask = 0;
    }

    inline void record(Hop hop) {
        if (__builtin_expect(!in_trace_, 0)) return;
        auto pos = write_pos();
    ...
    size_t count() const {
        return total_count_ < capacity_ ? total_count_ : capacity_;
    }
